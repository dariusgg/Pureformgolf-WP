...
if ( ! class_exists( 'WC_Gateway_EWAY' ) ) {
	...
	class WC_Gateway_EWAY extends WC_Payment_Gateway {
		...
		protected function process_payment_request( $order, $amount_to_charge, $eway_token_customer_id ) {
			...
			if ( intval( $result->Payment->InvoiceReference ) !== $order->id ) {
		...
		public function receipt_page( $order_id ) {
			...
			try {
				...
				echo '<input type="hidden" name="EWAY_CARDNAME" value="' . esc_attr( $order->billing_first_name . ' ' . $order->billing_last_name ) . '"/>';
				...
				$form = '<form method="post" action="' . esc_url( $form_url ) . '" id="eway_credit_card_form">';
				$form .= ob_get_clean();
				$form .= '</form>';
				echo $form;
			} catch ( Exception $e ) {
		...
		protected function get_token_customer_id( $order ) {
			...
			return get_post_meta( $order->id, '_eway_token_customer_id', true );
		...
		protected function set_token_customer_id( $order, $token_customer_id ) {
			...
			update_post_meta( $order->id, '_eway_token_customer_id', $token_customer_id );
