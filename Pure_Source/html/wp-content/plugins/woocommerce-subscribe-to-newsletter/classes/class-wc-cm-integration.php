...
class WC_CM_Integration {
	...
	public function subscribe( $first_name, $last_name, $email, $listid = 'false' ) {
		...
		if ( ! $result->was_successful() ) {
			// Email admin
			wp_mail( get_option( 'admin_email' ), esc_html__( 'Email subscription failed (Campaign Monitor)', 'wc_subscribe_to_newsletter' ), '(' . esc_html( $result->http_status_code ) . ') ' . print_r( $result->response, true ) );
		} else {
