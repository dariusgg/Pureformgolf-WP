...
class WC_Mailpoet_Integration {
	...
	public function show_stats() {
		...
		$stats .= '<li><strong style="font-size:3em;">' . esc_html( $counts['subscribed'] ) . '</strong> ' . esc_html__( 'Total subscribers', 'wc_subscribe_to_newsletter' ) . '</li>';
		$stats .= '<li><strong style="font-size:3em;">' . esc_html( $counts['unsubscribed'] ) . '</strong> ' . esc_html__( 'Unsubscribes', 'wc_subscribe_to_newsletter' ) . '</li>';
		$stats .= '</ul>';
	...
	public function subscribe( $first_name, $last_name, $email, $listid = false ) {
		...
		if ( false === $add_subscriber ) {
			...
			if ( is_array( $messages ) && is_array( $messages['error'] ) ) {
				...
				$response = esc_html__( 'No error messages were returned by MailPoet.', 'wc_subscribe_to_newsletter' );
			}

			wp_mail(
				get_option( 'admin_email' ),
				esc_html__( 'Email subscription failed (MailPoet)', 'wc_subscribe_to_newsletter' ),
				esc_html__( 'Error:', 'wc_subscribe_to_newsletter' ) . "\n\n" . $response
			);
