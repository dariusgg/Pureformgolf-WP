...
abstract class WP_Async_Task {
	...
	public function launch() {
		...
		if ( ! has_action( 'shutdown', array( $this, 'launch_on_shutdown' ), PHP_INT_MAX ) ) {
			add_action( 'shutdown', array( $this, 'launch_on_shutdown' ), PHP_INT_MAX );
		}
	}
		...
		if ( ! empty( $this->_body_data ) ) {
			...
			$request_args = array(
				...
				'headers'   => array(
            		...
            		$url = preg_replace( '#(http[s]?)://([^/]+)#', '$1://' . $_SERVER['SERVER_ADDR'], admin_url( 'admin-post.php' ) );
	...
	public function handle_postback() {
		...
		add_filter( 'wp_die_handler', create_function( '', "die();") );
		wp_die();
	}
